...
A machine stows the shared packages it wants alongside its own. Shared holds the base config and the machine overrides it through the optional file each format already looks for - `config.local`, `.zshrc.local`, an `Include` fragment, an `includeIf` path - which is ignored when absent.
