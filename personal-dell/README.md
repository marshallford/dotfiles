...
pacman -S intel-media-driver vpl-gpu-rt mesa vulkan-intel
pacman -S vim less wget btop htop zip unzip zsh fwupd udisks2 usbutils stow kernel-modules-hook
...
```shell
yay -S noto-fonts noto-fonts-cjk noto-fonts-emoji noto-fonts-extra ttf-jetbrains-mono-nerd
```

Windows fonts

```shell
yay -S ttf-ms-win11 # should fail
...
```shell
yay -S wireplumber pipewire pipewire-alsa pipewire-jack pipewire-pulse
```
...
```shell
yay -S dms-shell-niri dgop nautilus xdg-desktop-portal-gtk xdg-desktop-portal-gnome gnome-keyring xwayland-satellite libappindicator wl-clipboard cava i2c-tools matugen power-profiles-daemon qt6-multimedia-ffmpeg qt6ct wtype adw-gtk-theme cups-pk-helper kimageformats
...
```shell
yay -S vlc vlc-plugins-all chromium ghostty visual-studio-code-bin bruno-bin spotify-launcher slack-desktop discord loupe file-roller baobab seahorse gnome-calculator
```
